...
void generateCombinationsHelper(const unordered_set<unsigned int>& nums, int k, int start, vector<int>& currentCombination, vector<vector<int>>& combinations) {
...
//TODO change this into boost::
vector<vector<int>> generateCombinations(const unordered_set<unsigned int>& nums, int k) {
...
{    
    ...
    for (unsigned int n = 1; n <= num_activations; ++n)
    {
        ...
        const vector<vector<int>> combinations = generateCombinations(valid_hydrogens, n); // change into unsigned ints

        // Iterate over each combination of activations
        for (const vector<int>& activation_pattern : combinations) // 
        {
            ...
            for (const int& activate_idx : activation_pattern) // activate_idx should not be unique, two activations on 1 atom
            {
...
{    
    ...
    {
        // Generate all combinations to mutate input molecule N times
        const vector<vector<int>> combinations = generateCombinations(valid_bond_indices, n); // change into unsigned ints
...
{    
    ...
    for (unsigned int n = 1; n <= num_activations; ++n)
    {
        ...
        // Generate all combinations to mutate input molecule N times
        const vector<vector<int>> combinations = generateCombinations(valid_spiro_indices, n); // change into unsigned ints
