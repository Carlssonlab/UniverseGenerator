...
bool is_activated_bond(const RDKit::ROMol &mol, const RDKit::Bond &bond)
{
    ...
    std::unordered_set<unsigned int> activation_tags = {74u,75u,76u};
...
bool is_ketene(const RDKit::ROMol &mol)
{
    const RDKit::RWMol *pattern = RDKit::SmartsToMol("[Hf]=[C]=[A]");

    std::vector<RDKit::MatchVectType> matches;    

    RDKit::SubstructMatch(mol, *pattern, matches);

    const unsigned int num_matches = matches.size();
...
int main(int argc, char *argv[])
{
    ...
    const std::string usage_string = " -i <input_file_path> -o <output_file_path> -l <label>";

    // Argument parser
    int opt;
    while ((opt = getopt(argc, argv, "i:o:l:v:h")) != -1)
    {
        switch (opt)
        {
            case 'i':
    ...
    while (std::getline(input_stream, line))
    {
        ...
        for (unsigned int bond_idx = 0 ; bond_idx < num_bonds ; ++bond_idx) 
        {
            ...
            if (is_activated_bond(*mol, *bond) && ring_info->numBondRings(bond_idx))
            {
                output_stream << line + "\n";
            }
        }
        ...
        */
        ...
        // Remove a specific chemical pattern that we allowed to be made during the activations
        if (!is_ketene(*mol))
        {
            output_stream << line + "\n";
        }
